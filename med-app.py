...
common_interactions = {
    ...
    ("atorvastatin",
     "grapefruit"): "Grapefruit inhibits CYP3A4, which may increase atorvastatin levels → risk of side effects like stomach damage",
    ...
    ("Ciprofloxacin",
     "Tizanidine"): "Ciprofloxacin inhibits CYP1A2, increasing tizanidine levels and risk of hypotension",
    ...
    ("grapefruit juice",
     "statins"): "Grapefruit juice inhibits CYP3A4, increasing statin levels and risk of side effects",
...
# Toggle for interaction checking
st.sidebar.header("Settings")
st.session_state.check_interactions = st.sidebar.toggle(
...
if st.session_state.check_interactions:
    st.sidebar.info("🔍 Interaction checking is enabled")
else:
    st.sidebar.info("⚡ Interaction checking is disabled")
...
# In your settings sidebar section:
st.sidebar.header("Notification Settings")

# User can enter their email for notifications
user_email = st.sidebar.text_input("Your email for notifications", value=config["to_email"])

# Update the config with user's email
if user_email and user_email != config["to_email"]:
    config["to_email"] = user_email
    st.sidebar.success(f"Notifications will be sent to {user_email}")